...
- Public upload settings known via environment variables when using cloud upload:
  - `PI_VIDEO_RCLONE_DEST` (example: `r2:my-bucket/pr-videos`)
  - `PI_VIDEO_PUBLIC_BASE_URL` (example: `https://<public-domain>.r2.dev/pr-videos`)
</requirements>
...
# Configure destination. Examples:
#   export PI_VIDEO_RCLONE_DEST="r2:my-bucket/pr-videos"
#   export PI_VIDEO_PUBLIC_BASE_URL="https://<public-domain>.r2.dev/pr-videos"
: "${PI_VIDEO_RCLONE_DEST:?Set PI_VIDEO_RCLONE_DEST, e.g. r2:my-bucket/pr-videos}"
: "${PI_VIDEO_PUBLIC_BASE_URL:?Set PI_VIDEO_PUBLIC_BASE_URL, e.g. https://domain/pr-videos}"

DEST="${PI_VIDEO_RCLONE_DEST%/}/pr-[number]"
PUBLIC_BASE_URL="${PI_VIDEO_PUBLIC_BASE_URL%/}"

# Upload video, preview GIF, and screenshots to cloud storage
# Use --s3-no-check-bucket for S3-compatible remotes that require it.
rclone copy tmp/videos/ "$DEST/" --s3-no-check-bucket --progress
rclone copy tmp/screenshots/ "$DEST/screenshots/" --s3-no-check-bucket --progress

# List uploaded files
rclone ls "$DEST/"

# Build and validate public URLs BEFORE updating PR
VIDEO_URL="$PUBLIC_BASE_URL/pr-[number]/feature-demo.mp4"
PREVIEW_URL="$PUBLIC_BASE_URL/pr-[number]/feature-demo-preview.gif"
