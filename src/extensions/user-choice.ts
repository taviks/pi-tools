...
const UserChoiceParamsSchema = Type.Object({
	...
	options: Type.Array(ChoiceOptionSchema, {
		description: "Options the user can select. Keep this list small.",
	}),
	...
	includeOther: Type.Optional(
		Type.Boolean({
			description:
				"Append a default 'Something else (type)' option. Default true.",
		}),
	),
				...
				result = await ctx.ui.custom<UserChoiceResult>(
					(tui, theme, _keybindings, done) => {
						...
						function optionTag(
							...
							const label = option.input.required
								? "[type required]"
								: "[type]"
						...
						function renderOption(
							...
							lines.push(
								truncateToWidth(
									`${prefix}${index + 1}. ${optionTag(option, isSelected)} ${labelText}`,
									width,
								),
							)
						...
						function render(width: number): string[] {
							...
							if (editMode) {
								...
								add(
									theme.fg(
										"dim",
										` Enter submit text • Esc back to choices${cancelText}`,
									),
								)
								...
								add(
									theme.fg(
										"dim",
										` Enter start typing • or just type • ↑↓ move${cancelText}`,
									),
								)
							} else {
								const cancelText = allowCancel ? " • Esc cancel" : ""
								add(
									theme.fg(
										"dim",
										` Enter choose selected • ↑↓ move${cancelText}`,
									),
								)
							}
							add(border)
